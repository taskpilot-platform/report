...
[Hình x: Sơ đồ Deployment Architecture tổng quan của TaskPilot]

| Thành phần        | Nền tảng triển khai/dịch vụ                                | Vai trò                                                                     |
| ----------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------- |
...
[Hình x: Luồng kết nối giữa Frontend, Backend và dịch vụ ngoài]

### 3.13.2. Frontend trên Netlify và Vercel
...
Quy trình tạo backend container được chia thành hai giai đoạn như Bảng x.

| Giai đoạn | Base image | Xử lý chính | Kết quả |
| --- | --- | --- | --- |
...
Bảng x: Hai giai đoạn tạo backend Docker image
...
Bảng x: Một số lệnh chính trong backend Dockerfile
...
[Hình x: Quy trình build và chạy backend container trên Hugging Face Space]
...
[Hình x: Luồng gửi email đặt lại mật khẩu qua Brevo]
...
[Hình x: Luồng gửi push notification qua OneSignal]
...
### 3.13.8. Docker và CI/CD với GitHub Actions
...
[Hình x: Quy trình CI/CD với GitHub Actions]
