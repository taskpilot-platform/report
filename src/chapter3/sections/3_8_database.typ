...
#db-table-figure(
  ...
  column(
    [`created_at`],
    [`timestamp with time zone`],
    [NOT NULL, DEFAULT `CURRENT_` \ `TIMESTAMP`],
...
#db-table-figure(
  ...
  column([`performance _score`], [`double precision`], [DEFAULT `0.5`], [Điểm
...
#db-table-figure(
  ...
  column(
    [`created_at`],
    [`timestamp with time zone`],
    [INDEX, DEFAULT `CURRENT_` \ `TIMESTAMP`],
...
#db-table-figure(
  ...
  column([`type`], [`public.` \ `notification_` \ `type`], [], [Loại thông báo,
    ví dụ system, assigned, comment, mention hoặc reply.]),
