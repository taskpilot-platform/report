...
Các AI provider chỉ tham gia ở vai trò sinh phản hồi hoặc đề xuất lời gọi tool
có cấu trúc. Provider không truy cập trực tiếp cơ sở dữ liệu và không tự ý thay
đổi dữ liệu nghiệp vụ. Mọi dữ liệu đưa vào ngữ cảnh, mọi tool được công bố cho
model và mọi thao tác có tác động đến project đều phải đi qua các service,
port/adapter và rule nghiệp vụ của backend.

=== Kiến trúc và quản lý ngữ cảnh

Kiến trúc AI Copilot được tổ chức theo hướng backend-mediated: frontend gửi yêu
cầu hội thoại, backend tiếp nhận và điều phối các bước xử lý, còn AI provider
chỉ được gọi sau khi hệ thống đã xác định ngữ cảnh và quyền truy cập phù hợp.
Cách tổ chức này giúp AI Copilot hỗ trợ linh hoạt cho người dùng nhưng vẫn giữ
ranh giới an toàn đối với dữ liệu project và các quy tắc nghiệp vụ.
...
hệ thống. Khi người dùng gửi tin nhắn, backend ghi nhận message trước, sau đó
mới chuyển sang luồng xử lý AI. Nhờ vậy, lịch sử hội thoại vẫn có vết lưu trữ kể
cả khi provider phản hồi lỗi hoặc quá trình streaming bị gián đoạn.
...
lưu snapshot bộ nhớ hội thoại theo session, giúp backend tái sử dụng phần lịch
sử phù hợp thay vì nạp lại toàn bộ nội dung cũ trong mỗi lần gọi model.

Ngữ cảnh gửi sang model được xây dựng từ nhiều lớp thông tin: system
instruction, lịch sử hội thoại gần đây, snapshot bộ nhớ, dữ liệu project/task
cần thiết và kết quả tool nếu có. Khi hội thoại dài, backend có thể rút gọn phần
lịch sử cũ và chỉ giữ lại những thông tin còn liên quan đến câu hỏi hiện tại.
Các phần giải thích hoặc cơ sở phản hồi được trình bày ở mức cần thiết cho người
dùng, trong khi metadata xử lý được dùng cho vận hành và truy vết.
...
ngữ cảnh, nhu cầu dùng tool và cấu hình hiện tại của hệ thống. Ví dụ, yêu cầu
hỏi đáp ngắn có thể đi qua route nhẹ, còn yêu cầu phân tích workload, gợi ý phân
công hoặc cần tool calling có thể được chuyển sang route có cấu hình phù hợp
hơn.

Trong thiết kế hiện tại, bước nhận diện ban đầu có thể dùng một provider phản
hồi nhanh để phân loại ý định, sau đó backend chọn provider hoặc model chính
theo rule cấu hình. Cơ chế này mang tính Strategy-like routing: hệ thống có
nhiều chiến lược xử lý và chọn chiến lược phù hợp cho từng nhóm yêu cầu, nhưng
quyết định cuối cùng vẫn do backend điều phối.
...
năng đã được định nghĩa, chẳng hạn truy vấn thông tin project, lấy danh sách
task, phân tích workload hoặc chuẩn bị thao tác phân công. Để kiểm soát bề mặt
chức năng này, TaskPilot sử dụng Tool Calling Registry. Chỉ những tool được
backend đăng ký, mô tả và công bố trong registry mới có thể xuất hiện trong AI
workflow.

Registry lưu thông tin về tên tool, mục đích, tham số đầu vào, dạng dữ liệu đầu
ra và nhóm kiểm soát tương ứng. Tool không phải đường truy cập trực tiếp xuống
cơ sở dữ liệu. Mỗi lời gọi tool vẫn đi qua service, port/adapter và rule phân
quyền của backend. Nhờ đó, AI provider chỉ đề xuất lời gọi có cấu trúc, còn
backend quyết định lời gọi đó có hợp lệ và có được thực thi hay không.
...
authorization context hiện tại. Write tools không được thực thi ngay khi model
đề xuất. Thay vào đó, backend tạo một pending action gồm mô tả thao tác, tham số
dự kiến, preview nếu cần, user, session và thời hạn hiệu lực. Frontend hiển thị
...
thời hạn hiệu lực, quyền project, trạng thái dữ liệu và domain rules. Chỉ khi
tất cả điều kiện hợp lệ, domain service tương ứng mới thực hiện thao tác ghi như
tạo task, cập nhật task hoặc phân công thành viên. Nếu người dùng hủy hoặc
pending action hết hạn, backend không thực hiện thay đổi dữ liệu.
...
cuối cùng vẫn là nghiệp vụ backend thông thường với đầy đủ kiểm tra quyền và
điều kiện miền.
...
project, task, member hoặc comment chỉ được đưa vào AI workflow khi người dùng
có quyền tương ứng.
...
thao tác được xác nhận, domain validation vẫn kiểm tra tham số, trạng thái
project và các rule nghiệp vụ trước khi ghi dữ liệu. Các lớp này giúp AI Copilot
hoạt động như một lớp hỗ trợ được backend trung gian hóa, không phải một tác
nhân tự do thao tác lên hệ thống.
...
Về mẫu thiết kế, Tool Calling Registry thể hiện Registry Pattern vì các tool
được quản lý tập trung và công bố cho AI workflow dưới dạng specification thống
nhất. Smart routing mang tính Strategy-like khi backend chọn đường xử lý hoặc
provider theo loại yêu cầu và cấu hình hiện tại. Ở ranh giới tích hợp, Adapter
Pattern được dùng để cô lập khác biệt giữa các AI provider và giữa module AI với
các module nghiệp vụ thông qua contract/port. Những pattern này giúp AI Copilot
có thể mở rộng mà vẫn giữ ranh giới trách nhiệm rõ ràng.
