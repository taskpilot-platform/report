...
Trong phần scoring hiện tại, `L(u)` được giữ như workload cost và bị trừ ở công
thức cuối. Nếu `Xmax = Xmin` đối với một tiêu chí, hệ thống gán cùng một giá trị
chuẩn hóa cho tất cả ứng viên ở tiêu chí đó, vì tiêu chí này không tạo ra khác
biệt xếp hạng trong lần tính hiện tại.
...
thiết kế. Ba tiêu chí được đưa vào so sánh là workload cost `L(u)`, skill fit
`F(u,t)` và performance `P(u)`. Thứ tự tiêu chí trong các ma trận dưới đây là
...
#figure(
  ...
  caption: [Kết quả trọng số AHP cho workload cost, skill fit và performance ở mode BALANCED],
)
...
#figure(
  ...
  caption: [Kết quả trọng số AHP cho workload cost, skill fit và performance ở mode URGENT],
)
...
#figure(
  ...
  caption: [Kết quả trọng số AHP cho workload cost, skill fit và performance ở mode TRAINING],
)
...
score, `L(u)` là workload cost và `P(u)` là performance score. `L(u)` càng cao
nghĩa là ứng viên đang có workload hiện tại càng nặng. Dấu trừ trước
`w_load x L(u)` thể hiện đặc trưng của workload cost: workload cao là bất lợi
trong phân công thêm task.
...
#ui-table-figure(
  ...
  text(size: 10pt)[
    #table(
      columns: (1.1fr, 0.65fr, 0.65fr, 0.65fr, 2.8fr, 0.6fr, 0.55fr),
    align: (
      left + top,
      center + top,
      center + top,
