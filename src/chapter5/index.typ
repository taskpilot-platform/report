...
tập trung vào những workflow chính của đồ án, không đặt mục tiêu thay thế toàn bộ
các nền tảng chuyên sâu như Jira hoặc Asana.

Hệ thống cũng hỗ trợ cộng tác thông qua bình luận, mention và notification. Người
dùng có thể trao đổi trên từng task bằng comment, phản hồi theo luồng và nhắc tên
thành viên liên quan. Khi có sự kiện như bình luận, mention hoặc phân công công
việc, hệ thống có thể tạo thông báo trong ứng dụng, cập nhật realtime bằng SSE và
gửi push notification qua OneSignal khi cấu hình khả dụng.
...
Frontend của hệ thống được xây dựng dưới dạng React + TypeScript + Vite SPA. Cách
tiếp cận ứng dụng trang đơn giúp giao diện phản hồi nhanh, phù hợp với các màn
hình như danh sách dự án, workspace, Kanban board, notification và AI chat.
...
Backend được triển khai bằng Spring Boot theo kiến trúc Modular Monolith. Toàn bộ
backend chạy trong một ứng dụng/runtime duy nhất, trong khi mã nguồn được tổ chức
...
chính như người dùng, project, sprint, task, comment, notification, phiên chat AI
và log hoạt động AI. Lược đồ cơ sở dữ liệu được quản lý bằng Flyway migration,
giúp theo dõi thay đổi schema theo phiên bản và đồng bộ cấu trúc dữ liệu giữa các
môi trường. Một số dữ liệu linh hoạt như cấu hình hệ thống hoặc kết quả tool AI
được lưu dưới dạng JSONB.
...
mô đồ án. Frontend React/Vite được build thành tài nguyên tĩnh và triển khai trên
Vercel/Netlify theo cấu hình dự án. Backend Spring Boot được đóng gói bằng Docker
và triển khai trên Hugging Face Space, phù hợp với mô hình một ứng dụng backend
chạy tập trung.
...
Face Space. Tuy nhiên, phạm vi kiểm thử tự động trong pipeline vẫn còn hạn chế và
cần được mở rộng thêm. Các lựa chọn triển khai này mang tính thực dụng, phù hợp
với ngân sách và phạm vi của đồ án sinh viên, chưa hướng đến mô hình vận hành có
độ sẵn sàng cao hoặc tiêu chuẩn doanh nghiệp.
...
Khó khăn đầu tiên nằm ở việc thiết kế ranh giới module trong khi backend vẫn chạy
như một ứng dụng duy nhất. Các module Users, Projects và AI có quan hệ dữ liệu
chặt chẽ, đặc biệt khi AI Copilot cần thông tin về project, task, thành viên, kỹ
năng và workload. Việc giữ ranh giới module đủ rõ nhưng không làm tăng độ phức
tạp quá mức là một vấn đề cần cân nhắc trong quá trình triển khai.
...
trung gian, đồng thời đảm bảo các hành động ghi dữ liệu vẫn đi qua kiểm tra quyền
và xác nhận của người dùng.
...
Việc triển khai trên các nền tảng thực dụng hoặc free-tier cũng tạo ra một số hạn
chế trong quá trình vận hành thử nghiệm. Backend trên Hugging Face Space có thể
gặp độ trễ khởi động hoặc giới hạn tài nguyên; các dịch vụ AI có thể gặp rate
limit; frontend và backend cần cấu hình CORS, biến môi trường và endpoint chính
xác. Bên cạnh đó, do thời gian đồ án có hạn, kiểm thử tự động mới ở mức hạn chế,
chưa bao phủ đầy đủ toàn bộ luồng nghiệp vụ và giao diện.
...
nghiệm tương tác; backend xử lý nghiệp vụ, xác thực, phân quyền, lưu trữ dữ liệu,
realtime và AI. Sự phân tách này giúp mỗi phần có thể được kiểm thử, cấu hình và
triển khai theo cách phù hợp hơn.
...
phạm vi backend và chịu ràng buộc bởi quyền của người dùng. Đối với write action,
cơ chế pending confirmation giúp người dùng giữ quyền quyết định cuối cùng trước
...
push notification, hệ thống đáp ứng được nhu cầu cập nhật thông tin trong phạm vi
cộng tác nhóm của đồ án.

=== Hạn chế

Hạn chế hiện tại của hệ thống là chưa triển khai RAG hoặc cơ chế truy xuất tài
liệu bên ngoài cho AI Copilot. Ngữ cảnh AI chủ yếu dựa trên lịch sử hội thoại, dữ
liệu hệ thống được truy vấn qua tool và cơ chế rút gọn, kiểm soát ngữ cảnh. Vì
vậy, AI chưa có khả năng tìm kiếm tri thức từ kho tài liệu hoặc vector store như
một chức năng hoàn chỉnh.

Cơ chế routing/fallback giữa các AI provider vẫn dựa trên luật và cấu hình, chưa
phải hệ thống học thích nghi theo hiệu quả sử dụng. Khi provider gặp lỗi, timeout
hoặc giới hạn tốc độ, fallback chỉ giúp tăng khả năng phục hồi ở mức nhất định.
Hệ thống chưa tự học để tối ưu lựa chọn provider/model dựa trên dữ liệu vận hành
dài hạn.
...
ban đầu, nhưng khi áp dụng vào các nhóm có đặc thù khác nhau, các trọng số này có
thể cần điều chỉnh. Hiệu quả của kết quả gợi ý phụ thuộc nhiều vào chất lượng dữ
liệu kỹ năng, workload và performance được ghi nhận trong hệ thống.
...
Việc triển khai dựa trên các nền tảng thực dụng/free-tier giúp giảm chi phí nhưng
cũng kéo theo giới hạn về hiệu năng, cold start, tài nguyên và độ ổn định.
...
Các hạn chế này không phủ nhận kết quả đã đạt được, mà phản ánh ranh giới phạm vi
hiện tại và các hướng cải thiện trong tương lai.

== Hướng phát triển

Mặc dù phiên bản hiện tại của TaskPilot đã đạt được các mục tiêu chính trong phạm
vi đã xác định, cung cấp một hệ thống quản lý dự án tích hợp AI có khả năng hỗ
trợ quy trình làm việc, hệ thống vẫn còn những giới hạn nhất định. Các phân tích
ở phần hạn chế cho thấy không gian để tiếp tục nâng cấp hệ thống. Một hướng phát
triển tiếp theo là tập trung cải thiện tính thông minh, khả năng cộng tác, độ tin
cậy, quy trình kiểm thử và bảo mật. Các đề xuất dưới đây là những hướng đi khả
thi nhằm hoàn thiện hệ thống trong tương lai.
...
người dùng trên hệ thống. Dữ liệu này được trích xuất từ cơ sở dữ liệu quan hệ và
cung cấp cho mô hình thông qua các công cụ. Tuy nhiên, trong thực tế quản lý dự
án, các nhóm làm việc thường tạo ra rất nhiều tài liệu phi cấu trúc như đặc tả
yêu cầu, biên bản họp, tài liệu thiết kế hoặc các tệp đính kèm.

Một hướng phát triển tiếp theo là tích hợp kỹ thuật Retrieval-Augmented Generation
(RAG) để cho phép AI có khả năng đọc và trả lời câu hỏi dựa trên các tài liệu nội
bộ dự án. Khi được triển khai, hệ thống sẽ có thêm chức năng trích xuất nội dung
từ các tệp tài liệu, phân mảnh (chunking) văn bản và nhúng (embedding) thành các
vector để lưu trữ vào một vector database hoặc cơ chế lưu trữ embedding phù hợp.
...
chấp nhận hoặc từ chối đề xuất phân công của AI, các quyết định gán việc thủ công
khác với đề xuất, cũng như kết quả hoàn thành của công việc đó.

Từ tập dữ liệu phản hồi này, hệ thống sẽ tự động điều chỉnh các trọng số hoặc
tinh chỉnh chiến lược phân công cho phù hợp với phong cách quản lý và đặc thù của
từng dự án. Thay vì sử dụng một bộ trọng số tĩnh cho tất cả các tình huống, hệ
thống có thể tinh chỉnh cách đánh giá mức độ phù hợp của từng thành viên dựa trên
dữ liệu quá khứ và phản hồi thực tế từ PM.
...
độ dự án, đảm bảo tính minh bạch và có thể giải thích được. Người quản lý vẫn giữ
quyền quyết định cuối cùng và có thể điều chỉnh hoặc đặt lại trọng số khi cần.
...
Để đảm bảo an toàn, các hành động làm thay đổi trạng thái dữ liệu (write actions)
được sinh ra từ lệnh giọng nói vẫn phải tuân thủ nghiêm ngặt quy trình xác nhận
hiện tại. Hệ thống sẽ hiển thị giao diện yêu cầu xác nhận để người dùng kiểm tra
lại thông số trước khi thực thi, ngăn chặn các thao tác sai lệch do lỗi nhận dạng
giọng nói.

=== WebRTC / Video call

Tính năng gọi điện video và âm thanh thời gian thực có thể hỗ trợ tốt hơn cho quá
trình cộng tác trong không gian dự án. Trong tương lai, hệ thống có thể được tích
hợp công nghệ WebRTC để hỗ trợ liên lạc trực tiếp giữa các thành viên thông qua
kết nối ngang hàng (peer-to-peer) hoặc có sự hỗ trợ của máy chủ phân phối
(server-assisted).

Việc tích hợp này sẽ giúp người dùng có thể thảo luận trực tiếp trong ngữ cảnh
của một task, sprint hoặc cuộc họp dự án. Thông tin ngữ cảnh từ màn hình hiện tại
có thể dễ dàng chia sẻ giữa những người tham gia cuộc gọi.

Để triển khai hướng này, hệ thống cần bổ sung các hạ tầng liên quan đến máy chủ
báo hiệu (signaling server) để thiết lập kết nối, quản lý băng thông và xử lý các
vấn đề mạng phức tạp. Đồng thời, cơ chế kiểm tra quyền truy cập phòng họp và bảo
vệ luồng truyền thông cũng phải được thiết kế chặt chẽ.
...
là mở rộng độ bao phủ của bộ kiểm thử tự động để đảm bảo độ tin cậy và sự ổn định
...
Bên cạnh đó, integration test nên được xây dựng cho các luồng auth, project/task,
realtime và AI để giảm rủi ro hồi quy.

Ở phía frontend, kiểm thử tự động sẽ tập trung vào kiểm thử thành phần (component
tests) cho các giao diện tái sử dụng, kiểm thử luồng người dùng (user-flow tests),
xác thực biểu mẫu và tính đúng đắn của các route được bảo vệ.

Các bộ kiểm thử này có thể được tự động hóa thông qua các công cụ CI/CD trước mỗi
lần triển khai. Việc tự động chạy các bài kiểm thử đối với mã nguồn mới sẽ giúp
phát hiện sớm các lỗi hồi quy, từ đó tăng cường độ tin cậy của phần mềm khi hệ
thống mở rộng quy mô.

=== Lưu trữ bền vững cho pending AI actions

Hiện tại, quy trình chờ xác nhận cho các hành động thay đổi dữ liệu của AI được
quản lý ở tầng workflow ứng dụng hoặc trạng thái tạm thời phía client/server. Nếu
người dùng tải lại trang, khởi động lại backend hoặc có sự cố mất kết nối mạng,
các trạng thái chờ xác nhận này có thể bị mất.

Để giải quyết vấn đề này, hệ thống có thể bổ sung cơ chế lưu trữ bền vững
(persistent storage) cho các hành động AI đang chờ xử lý. Khi AI đề xuất một hành
động thay đổi dữ liệu, thông tin về hành động đó sẽ được ghi nhận vào cơ sở dữ
liệu thay vì chỉ tồn tại tạm thời.
...
vai trò ngày càng quan trọng. Một hướng phát triển tiếp theo là thiết lập các quy
trình quản lý thông tin nhạy cảm (secrets) chặt chẽ hơn nhằm giảm thiểu rủi ro
rò rỉ.
...
quyền ở các tầng dịch vụ cũng là những công việc cần thiết để bảo vệ cấu hình ứng
dụng trước các rủi ro bảo mật.
...
việc thông thường, TaskPilot đã kết hợp các chức năng nghiệp vụ quản lý dự án với
khả năng hỗ trợ quy trình làm việc từ AI, giúp tự động hóa một số thao tác và
cung cấp thông tin ngữ cảnh nhanh chóng.
...
đồ án còn giúp hiểu rõ bài toán kiểm soát an toàn AI thông qua cơ chế yêu cầu xác
nhận của con người và những đánh đổi kỹ thuật khi triển khai phần mềm thực tế.
...
Những kinh nghiệm và kiến thức thu thập được từ quá trình nghiên cứu, thiết kế và
phát triển dự án này sẽ đóng vai trò làm nền tảng hữu ích cho quá trình học tập
và phát triển chuyên môn trong tương lai.
