...
trình duyệt qua kết nối HTTP liên tục [16]. TaskPilot dùng SSE cho các luồng cần
cập nhật tức thời từ server đến client như notification trong ứng dụng, comment
event và streaming phản hồi AI.

#figure(
  pad(bottom: -1.5em, image(
    "../../assets/taskpilot/chapter2/ch2_19_sse.svg",
    width: 100%,
  )),
...
giúp hệ thống nhắc người dùng về sự kiện quan trọng ngay cả khi họ không đang mở
màn hình ứng dụng.
...
Brevo là nền tảng gửi email giao dịch thông qua API hoặc SMTP [18]. TaskPilot sử
dụng Brevo cho các email hệ thống như khôi phục mật khẩu, nhằm tách chức năng
gửi email khỏi backend nghiệp vụ chính.
